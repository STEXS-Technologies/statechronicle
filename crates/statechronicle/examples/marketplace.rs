...
#[allow(clippy::too_many_arguments)]
fn settlement_intents(harness: &Harness, suffix: &str, version: u64) -> Vec<ValidatedIntent> {
...
async fn main() {
    ...
    let stale = settlement_intents(&harness, "bad", 99);
    ...
    let ok = settlement_intents(&harness, "ok", 1);
