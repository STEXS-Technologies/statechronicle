...
//! The accumulator produces level-tagged [`InclusionProof`](statechronicle_accumulator::proof::InclusionProof)s (ADR-005); the
//! domain's portable [`SparseMerkleProof`](statechronicle_domain::proof::SparseMerkleProof) wire form (§16.2) is untagged, so
//! this module converts between the two encodings.
//!
//! Wire encoding: the domain `path` is **dense**: exactly
//! [`TREE_DEPTH`](statechronicle_accumulator::sparse_merkle::TREE_DEPTH) sibling hashes in ascending level order (leaf-adjacent
