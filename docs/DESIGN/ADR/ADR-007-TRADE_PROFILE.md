...
| v2-2 | Cross-tenant settle, three legs | **Include after v2-1** (medium-large) | ✅ **Done (Phase 1, 3-tenant generalized manifest).** The manifest declares the settle/value-leg linkage and is enforced against the signed settle intents; `validate_cross_tenant_trade` admits N settle legs and M value legs |
