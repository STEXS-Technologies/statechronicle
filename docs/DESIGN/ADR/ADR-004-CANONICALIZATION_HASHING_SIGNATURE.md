...
Because the codebase is currently **doc stubs only** (no serialized bytes, no digests,
no persisted commits exist), switching the canonical form now costs nothing. After v0,
switching costs a schema migration and re-signing of real data.
...
- Integer amounts → canonical non-negative decimal integer strings (UTF-8) with fixed
  u64 precision in core transition arithmetic (checked add/sub, fail closed on
  overflow/underflow), e.g. `"125000"` with profile-defined `unit`/scale (§10.3).
...
- Criterion bench (already a workspace dep) on the encode+hash path to validate
  throughput claims.
...
- Golden vectors + TS/Go cross-checks + criterion bench before v0 freezes.
...
- **Change Log:**
  - v1.1 (2026-08-03): Baseline flipped from JSON + RFC 8785 JCS to BCS. JSON retained
    ...
    u128 economic newtypes; golden-vector conformance. Rationale: performance (§2.4/
