...
- stexs treats `trustgrant` as an in-repo crate but wires it through slice ports and
...
### Wiring Owned by stexs

- The `TrustGrantEvaluatorAdapter` — implementing `statechronicle_ports::TrustGrantEvaluator`
  against the real `trustgrant` crate — lives in the **stexs composition root** (extending
  the existing stexs `crates/api-http/src/infra/trustgrant/` pattern). stexs is the single
  place the three protocols meet; StateChronicle ships no adapter and no trustgrant dep.
- `statechronicle-http` v0 uses an in-memory fake, so all protocol logic is fully testable
  without the external workspace.
- **Deferred (tracked decision):** an optional `statechronicle-trustgrant` companion crate
  for standalone adopters who want real authority without a platform. Triggers: a second
  standalone consumer, trustgrant stabilized and published on crates.io, or the reference
  binary needing real authority out of the box. Not built now — trustgrant's API is still
  settling and churn would be wasted.
...
| **Ports-only, adapter in statechronicle-http** | Standalone binary works out of the box | Duplicates stexs' integration; makes the workspace unpublishable (path dep); two sources of truth for one consumer |
| **Optional companion crate (`statechronicle-trustgrant`)** | Standalone adopters get real authority; core stays clean | Churn against a settling API; deferred until triggers hit |
...
1. v0: in-memory fakes in statechronicle-http; protocol fully exercised.
2. v0.5: stexs implements `TrustGrantEvaluatorAdapter` at its composition root.
3. v1: revocation freshness policies, proof-bundle enrichment, key rotation via TrustGrant.
4. Triggered (not scheduled): `statechronicle-trustgrant` companion crate.
